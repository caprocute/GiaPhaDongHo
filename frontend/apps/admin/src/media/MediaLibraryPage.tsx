...
// ── CSS injection (hover + animation — không thể làm trong CSS tĩnh) ──────────
let _mlCss = false;
function ensureMediaCss() {
  ...
  s.textContent = `
    ...
    .ml-album-card:hover{border-color:var(--color-action-primary-bg)!important;box-shadow:var(--shadow-md)!important}
    .ml-drop:hover,.ml-drop.over{border-color:var(--color-action-primary-bg)!important;background:color-mix(in srgb,var(--color-action-primary-bg) 6%,transparent)!important}
    @keyframes ml-shimmer{0%{background-position:200% 0}100%{background-position:-200% 0}}
    .ml-skel{animation:ml-shimmer 1.5s ease infinite;background-size:200% 100%;background-image:linear-gradient(90deg,var(--color-surface-sunken) 0%,var(--color-surface-card) 50%,var(--color-surface-sunken) 100%)}
  `;
...
export function MediaLibraryPage() {
  ...
  async function doStartUpload() {
    ...
    const allItems = uploadQueue;
    for (let i = 0; i < allItems.length; i++) {
      if (allItems[i].status !== "pending") continue;
      setUploadQueue((prev) =>
        prev.map((it, idx) => (idx === i ? { ...it, status: "uploading" } : it)),
      );
      try {
        const result = await uploadMediaPhoto(
          allItems[i].file,
          { albumId, caption: uploadCaption },
          token,
        );
        setUploadQueue((prev) =>
          prev.map((it, idx) => (idx === i ? { ...it, status: "done", result } : it)),
        );
      } catch (e) {
    ...
    setUploading(false);
    setUploadCaption("");
    const done = uploadQueue.filter((u) => u.status === "done").length + 1;
    setToast(`Hoàn tất: ${done}/${pending.length} ảnh.`);
  ...
  async function doUpdateAlbum() {
    ...
    try {
      ...
      await updateMediaAlbum(
        editAlbum.id,
        { ...editAlbum, title: editTitle, description: editDesc || null },
        token,
      );
      setEditAlbum(null);
  ...
  const albumColumns: ProTableColumn<AlbumRow>[] = [
    ...
    },
    {
      ...
      sortable: true,
      render: (row) => (
        <strong style={{ fontFamily: "var(--font-display)" }}>{row.title as string}</strong>
      ),
      ...
      render: (row) => (
        <div style={{ display: "flex", gap: "var(--spacing-xs)" }}>
          <Button
            ...
            variant="secondary"
            onClick={() => {
              handleAlbumSelect(row.id as number);
              setTab("photos");
            }}
          >
            Xem ảnh
  ...
  return (
    <div className="admin-stack">
      ...
      <div className="ml-stats">
        {[
          ...
          { label: "Số album", value: albums.length, sub: "album sự kiện & tư liệu" },
          {
            ...
            sub: activeAlbumId
              ? `ảnh trong album · trang ${photoPage + 1}`
              : `trang ${photoPage + 1}`,
          },
          {
            label: "Đã chọn",
            value: selectedIds.size,
            sub: selectedIds.size > 0 ? "→ thanh thao tác" : "click ảnh để chọn",
          },
        ].map((c) => (
          <div
            key={c.label}
          ...
          >
            ...
            <div
              style={{
                ...
                fontSize: "22px",
                fontWeight: 700,
                color: c.accent ? "var(--color-action-primary-bg)" : "var(--color-text-primary)",
              }}
            >
      ...
      {/* ── Tab bar ── */}
      <div className="ml-tab-bar" role="tablist">
        {(
          [
            ["photos", "Ảnh"],
            ["albums", "Album"],
        ...
        <>
            ...
            {(() => {
              ...
              return (
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") handleAlbumSelect(null);
                  }}
                  style={{
                    flexShrink: 0,
                    width: 130,
                    borderRadius: "var(--radius-md)",
                    overflow: "hidden",
                    border: `2px solid ${isActive ? "var(--color-action-primary-bg)" : "var(--color-border-subtle)"}`,
                    cursor: "pointer",
                    boxShadow: "var(--shadow-sm)",
                ...
                >
                  ...
                  >
                    {[0, 1, 2, 3].map((j) => (
                      <div key={j} className="ml-skel" style={{ borderRadius: 2 }} />
                    ))}
                  </div>
                  <div
                    style={{
                ...
                <div
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ")
                      handleAlbumSelect(album.id ?? null);
                  }}
                  style={{
                    flexShrink: 0,
                    width: 130,
                    ...
                    {url ? (
                      ...
                      <div
                        style={{
                          width: "100%",
                          height: "100%",
                          background: `color-mix(in srgb, var(--color-action-primary-bg) ${60 - (i % 5) * 8}%, var(--color-surface-sunken))`,
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                          fontFamily: "var(--font-display)",
          ...
          >
            ...
            {activeAlbumId != null && (
              <span
                style={{
                  ...
                  gap: 4,
                  padding: "3px 10px",
                  background: `color-mix(in srgb,var(--color-action-primary-bg) 12%,var(--color-surface-card))`,
                  borderRadius: "var(--radius-sm)",
                  fontSize: "12px",
                  fontWeight: 600,
                  color: "var(--color-action-primary-bg)",
            ...
            >
              {loadingPhotos
                ? "Đang tải…"
                : `${photoTotal.toLocaleString("vi-VN")} ảnh · trang ${photoPage + 1}/${photoTotalPages}`}
            </span>
            <button
              type="button"
              onClick={() => void loadPhotos(activeAlbumId, photoPage, pageSize)}
              aria-label="Tải lại"
              style={{
                background: "var(--color-surface-card)",
                border: "1px solid var(--color-border-subtle)",
          ...
          {selectedIds.size > 0 && (
            <div
              style={{
                ...
                background: `color-mix(in srgb,var(--color-action-primary-bg) 8%,var(--color-surface-card))`,
                border: `1px solid color-mix(in srgb,var(--color-action-primary-bg) 30%,var(--color-border-subtle))`,
                borderRadius: "var(--radius-md)",
                marginBottom: "var(--spacing-md)",
                flexWrap: "wrap",
              }}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      overflow: "hidden",
                      cursor: "pointer",
                      border: `2px solid ${isSel ? "var(--color-action-primary-bg)" : "transparent"}`,
                      boxShadow: isSel
                        ? `0 0 0 3px color-mix(in srgb,var(--color-action-primary-bg) 25%,transparent)`
                        : "var(--shadow-sm)",
                      background: "var(--color-surface-sunken)",
                    ...
                    >
                      ...
                      >
                        <button
                          ...
                          style={{
                            ...
                            justifyContent: "center",
                            fontSize: "10px",
                            color: isSel ? "var(--color-action-primary-fg)" : "transparent",
                            flexShrink: 0,
                          }}
                        >
                          ✓
                        ...
                        <div style={{ display: "flex", gap: 4 }}>
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              void doDeletePhoto(photo.id);
                            }}
                            style={{
                              background: "rgb(180 0 0 / 60%)",
                              border: "1px solid rgb(255 100 100 / 40%)",
                              borderRadius: "var(--radius-sm)",
                              cursor: "pointer",
                              color: "white",
                              fontSize: "11px",
                      ...
                      <div
                        style={{
                          ...
                          fontWeight: 600,
                          lineHeight: 1.3,
                          textShadow: "0 1px 2px rgb(0 0 0 / 80%)",
                        }}
                      >
          ...
          >
                  ...
                  style={{
                    ...
                    borderColor:
                      s === pageSize
                        ...
                        : "transparent",
                    background:
                      s === pageSize ? "var(--color-action-primary-bg)" : "none",
                    color:
                      s === pageSize
            ...
            <Button
              type="button"
              variant="secondary"
              disabled={photoPage === 0 || loadingPhotos}
            ...
            <Button
              type="button"
              variant="secondary"
              disabled={photoPage >= photoTotalPages - 1 || loadingPhotos}
              onClick={() => handlePageChange(photoPage + 1)}
            >
              ›
            </Button>
            <Button
              ...
              variant="secondary"
              disabled={photoPage >= photoTotalPages - 1 || loadingPhotos}
              onClick={() => handlePageChange(photoTotalPages - 1)}
            >
              »
            </Button>
      ...
      {tab === "albums" && (
        <ProTable
          ...
          toolbar={{
            ...
            actions: (
              <Button type="button" onClick={() => setShowCreateAlbum(true)}>
                + Tạo album
              </Button>
            ),
          }}
          emptyState={{
            title: "Chưa có album",
            description: "Tạo album để tổ chức ảnh theo sự kiện.",
            action: (
              <Button type="button" onClick={() => setShowCreateAlbum(true)}>
                Tạo album mới
              </Button>
      ...
      {tab === "upload" && (
        ...
        >
          ...
          <div
            ...
            onClick={() => {
              if (!uploading) document.getElementById("ml-file-input")?.click();
            }}
            style={{
          ...
          >
            ...
            >
              {uploading ? (
                <svg
                  ...
                  style={{
                    color: "var(--color-action-primary-bg)",
                    animation: "ml-shimmer 1s linear infinite",
                  }}
                >
            ...
            {uploading ? (
              <>
                ...
                >
                  Đang tải lên {uploadDoneCount + uploadErrorCount + 1}/
                  {uploadQueue.length}…
                </div>
              ...
              <>
                ...
                <div style={{ fontSize: "13px", color: "var(--color-text-muted)" }}>
                  hoặc{" "}
                  <strong style={{ color: "var(--color-action-primary-bg)" }}>
                    chọn file
                  </strong>{" "}
                  — JPG, PNG, WebP · nhiều file cùng lúc
                </div>
              </>
            )}
            <input
            ...
            >
              ...
              >
                <span style={{ fontSize: "12px", fontWeight: 700 }}>
                  {uploadQueue.length} file · {uploadDoneCount} xong
                  {pendingCount > 0 ? ` · ${pendingCount} chờ` : ""}
                  {uploadErrorCount > 0 ? ` · ${uploadErrorCount} lỗi` : ""}
                </span>
                {!uploading && (
                  <button
                    type="button"
          ...
          {/* Nút tải lên — chỉ hiện khi có file chờ và chưa đang tải */}
          {pendingCount > 0 && !uploading && (
            <div
              style={{
                ...
                padding: "var(--spacing-sm) var(--spacing-md)",
                background: `color-mix(in srgb,var(--color-action-primary-bg) 8%,var(--color-surface-card))`,
                border: `1px solid color-mix(in srgb,var(--color-action-primary-bg) 25%,var(--color-border-subtle))`,
                borderRadius: "var(--radius-md)",
              }}
            >
              <span style={{ flex: 1, fontSize: "13px", color: "var(--color-text-muted)" }}>
                {pendingCount} ảnh sẵn sàng tải lên
              </span>
              <Button
                type="button"
                variant="secondary"
              ...
              <Button type="button" onClick={() => void doStartUpload()}>
      ...
      {lightboxPhoto != null &&
        (() => {
          ...
          return (
            ...
            >
              <div className="ml-lightbox-box">
                <div className="ml-lightbox-photo">
                  {src ? (
                    <img
                      src={src}
                      ...
                      onError={(e) => {
                        if (
                          lightboxPhoto.thumbUrl &&
                          e.currentTarget.src !== lightboxPhoto.thumbUrl
                        )
                          e.currentTarget.src = lightboxPhoto.thumbUrl;
                      }}
                    />
                  ...
                  <button
                    ...
                    style={{
                      ...
                      alignItems: "center",
                      justifyContent: "center",
                      opacity: lightboxIdx >= photos.length - 1 ? 0.25 : 1,
                    }}
                  >
                    ›
                  </button>
                ...
                <div className="ml-lightbox-sidebar">
                  <div>
                    <div
                      style={{
                        fontFamily: "var(--font-display)",
                  ...
                  <div style={{ height: 1, background: "var(--lb-divider)" }} />
                  <div>
                    <div
                      style={{
                        fontSize: "11px",
                    ...
                    {album ? (
                      ...
                      <span style={{ fontSize: "13px", color: "var(--lb-muted)" }}>
                        Chưa gắn album
                      </span>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                      <a
                        href={album ? `/photos/album-${album.id}/` : "/photos/"}
                        target="_blank"
                        rel="noreferrer"
                        style={{
                          fontSize: "12px",
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        padding: "8px",
                        borderRadius: "var(--radius-sm)",
                        border: "1px solid var(--lb-danger-bg)",
                        background: "var(--lb-danger-bg)",
                        color: "var(--lb-danger-fg)",
                        fontSize: "12px",
                        fontWeight: 600,
                        cursor: "pointer",
                      }}
      ...
      {editPhoto != null && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setEditPhoto(null);
          }}
        >
          <div className="ml-modal-box">
            {(editPhoto.thumbUrl ?? editPhoto.url) && (
              ...
              >
                <img
                  src={editPhoto.thumbUrl ?? editPhoto.url ?? ""}
                  alt=""
            ...
            >
              <button
                ...
                onClick={() => {
                  setEditPhoto(null);
                  void doDeletePhoto(editPhoto.id);
                }}
                style={{
                  background: "none",
                  border: "none",
      ...
      {showCreateAlbum && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setShowCreateAlbum(false);
          }}
        >
          <div className="ml-modal-box">
            <h2
      ...
      {editAlbum != null && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setEditAlbum(null);
          }}
        >
          <div className="ml-modal-box">
            <h2
